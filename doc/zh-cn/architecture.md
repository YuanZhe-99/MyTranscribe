...
现有功能为 `jobs`（转写）、`providers`（来源与模型）、`local`（在设备上运行的模型：它们的记录、模型
包、引擎与路线）、`transcript`（阅读结果）、`media`（FFmpeg）、`secrets`（API Key）与 `settings`。

本地模型通过 `LocalTranscriptionBackend` 到达任务执行器，它使用唯一的引擎协议 `LocalAsrEngine`，每个
设备端适配器都实现这个协议。执行器的阶段状态机、续传与合并由上传任务和本地任务共用；参见
...
| 下载的模型 | 本地模型加载的模型包，以及本设备的引擎状态 | 否 —— 本地模型的*记录*作为配置同步 | 否 |

这些排除是**结构性**的，不是过滤出来的：同步、备份与 ZIP 引擎只会接触
`lib/app/data_modules.dart` 中注册表里的文件名，而密钥文件、任务目录和模型目录都不在其中。把其中任何
一个加入注册表，都会悄悄开始上传它。
