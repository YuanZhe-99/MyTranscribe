/// Purpose: Hand the Flutter tool sherpa-onnx's prebuilt C-API library and
/// the ONNX Runtime it needs, for the target it is building, as code assets.
...
/// Returns: Code assets — `sherpa`, the library the Dart bindings name, and
...
/// Notes: Nothing is compiled (decisions D21 and D22 of the local-models
/// plan). `native/binaries.json` names sherpa-onnx's own release assets: the
/// ASR-only shared libraries for Windows, macOS and Linux, and the Android
/// archive's `jni` folders. A target without an entry — iOS, for which
/// sherpa-onnx publishes no dynamic library — gets no assets, and the engine
/// reports itself as not built there. The same hook as `local_asr_whisper`'s,
/// with `.tar.bz2` added. See `doc/en-us/platform-notes.md`.
library;
    ...
    for (final file in files) {
      ...
      output.assets.code.add(
        CodeAsset(
          ...
          name: name == entry ? 'sherpa' : 'lib/$name',
...
/// Inputs: The [file]: a `.zip` (an Android `.aar` is one), a `.tar.gz` or a
/// `.tar.bz2`.
/// Returns: The archive.
