/// Purpose: Hand the Flutter tool the prebuilt Neural Engine bridge for the
/// Apple target it is building, as a code asset.
/// Inputs: The hook input: target OS, architecture and (on iOS) SDK, and a
/// shared output directory that caches the download between builds.
/// Returns: One code asset, `lasr_apple`, the library the Dart bindings name.
/// Side effects: Downloads the archive the first time, checks its SHA-256,
/// and unpacks the slice being built into the hook's shared output.
/// Notes: Nothing is compiled (decision D21 of the local-models plan): the
/// bridge — FluidAudio behind a C API — is built once by
/// `.github/workflows/apple-prebuild.yml`, and `native/binaries.json` pins its
/// release. Every target but macOS and iOS gets no assets, and the engine
    ...
    for (final file in files) {
      ...
      output.assets.code.add(
        CodeAsset(
          ...
          name: name == entry ? 'lasr_apple' : 'lib/$name',
